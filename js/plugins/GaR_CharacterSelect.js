 ...
 * @param AudioFolders
 * @type string
 * @default Harold,Therese
 * @desc Comma-separated audio folders matching ActorIds order.
 * @param StartMapIds
 * @type string
 * @default 3,3
...
(function () {
    ...
    function audioFolderForActor(actorId) {
        ...
        return AUDIO_FOLDERS[idx] || "";
    }
    // -------------------------------
    // Params
    // -------------------------------
    var ACTOR_IDS = parseCsvNums(params["ActorIds"], "1,2");
    var AUDIO_FOLDERS = String(params["AudioFolders"] || "")
        .split(",")
        .map(function (s) {
            return s.trim();
        });
    ...
    function setPlayerSpriteFromActorId(actorId) {
        ...
        if (name && $gamePlayer.setImage)
            $gamePlayer.setImage(name, index);
        if ($gamePlayer.refresh)
            $gamePlayer.refresh();
    }
    ...
    function forcePlayerVisible() {
        if (!$gamePlayer)
            return;
        if ($gamePlayer.setTransparent)
            $gamePlayer.setTransparent(false);
        if ($gamePlayer.setOpacity)
    ...
    function clampPlayerToMapOnce() {
        if (!$gameTemp || !$gameTemp._garClampOnce)
            return;
    ...
    // -------------------------------
    function Window_CharSelectList() {
    ...
    Window_CharSelectList.prototype.windowHeight = function () {
        if (this._forcedHeight && this._forcedHeight > 0)
    ...
    Window_CharSelectList.prototype.makeCommandList = function () {
        ...
        this._actorIds.forEach(function (id) {
            ...
            if (a)
                self.addCommand(a.name(), "actor", true, Number(id));
        });
    };
    ...
    Window_CharSelectList.prototype.update = function () {
        ...
        if (this._lastIndex !== this.index()) {
            ...
            if (this._onChange)
                this._onChange(this.currentActorId());
        }
    };
    ...
    Scene_CharacterSelect.prototype.onPickOk = function () {
        ...
        $gameSystem._selectedAudioActorId = this._pendingActorId;
        $gameSystem._selectedAudioFolder =
            audioFolderForActor(this._pendingActorId);
			console.log(
    "[GaR Audio] Selected:",
    this._pendingActorId,
    audioFolderForActor(this._pendingActorId)
);
        $gameSystem._selectedAudioActorId = this._pendingActorId;
...
Scene_CharacterSelect.prototype.onConfirmNo = function () {
    ...
    // Remove temporary audio override
    $gameSystem._selectedAudioActorId = 0;
    $gameSystem._selectedAudioFolder = "";
    ...
    // -------------------------------
    if (!GAR._origNewGame)
        GAR._origNewGame = Scene_Title.prototype.commandNewGame;
    ...
    // -------------------------------
    if (!GAR._origSceneMapStart)
        GAR._origSceneMapStart = Scene_Map.prototype.start;
...
// --------------------------------------------------
// Override static SEs (Cursor, OK, Cancel, etc.)
// --------------------------------------------------
function garCustomSe(se) {
    ...
    if (fs.existsSync(customPath)) {
        ...
        console.log(
            "[GaR Audio] Redirecting:",
            se.name,
            "->",
            customFolder + "/" + se.name
        );

        var copy = JSON.parse(JSON.stringify(se));
...
var _GAR_playSe = AudioManager.playSe;
...
var _GAR_playStaticSe = AudioManager.playStaticSe;
AudioManager.playStaticSe = function(se) {
    _GAR_playStaticSe.call(
        this,
        garCustomSe(se)
    );
};
